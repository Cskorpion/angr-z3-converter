...
from angrsmtdump.execution import Execution, ExecutionWriter, dump_executions, extract_all_regs_mem
...
def sim_and_dump_arm64(programs, filename=None, usepypcode=False, verbose=False):
    execs = []
    arch = archinfo.ArchAArch64() if usepypcode == False else archinfo.ArchPcode("AARCH64:LE:64:v8A")
    load_addr = 0
    ctr = 0
    for prog in programs:
        ...
        arch_descr = archinfo.ArchAArch64() if not usepypcode else "AARCH64:LE:64:v8A"
...
def sim_and_dump_rv64(programs, filename=None, usepypcode=False, verbose=False):
    ...
    ctr = 0
    filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "executions.py") if filename == None else filename
    excwriter = ExecutionWriter(filename)
    ...
    for prog in programs:
        ...
        exec = Execution(prog, arch_descr, 4, init_regs, init_mem, res_regs, res_mem, load_addr)
        excwriter.write(exec)

    excwriter.close()

def _sim_and_dump_rv64(programs, filename=None, usepypcode=False, verbose=False):
    execs = []
    arch = archinfo.ArchRISCV64() if (usepypcode == False) else archinfo.ArchPcode("RISCV:LE:64:RV64G")
    load_addr = 0
    ctr = 0
